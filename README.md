...
- **Regras Específicas por Colegiado**:
  - **Câmara de Julgamento (CJ)**: todo processo é Auto de Infração (campo pré-fixado e travado, eliminando erro de digitação); a 6ª coluna registra se houve **Defesa** (Sim/Não) — dado herdado pelos julgamentos do acervo; os destinos correspondem às cadeiras `CJ1`..`CJ5`, mapeadas para os conselheiros relatores via `cadeiras_cj`. Processo com **Defesa = "Não"** não participa do sorteio: vai sempre para a `CJ1`, que é quem recebe o lote de homologação de auto de infração — a regra vale mesmo com a `CJ1` excluída das pills, porque a exclusão é do sorteio e esse lote não passa por ele. O restante é sorteado normalmente entre as cadeiras participantes, e o lote sem defesa não desconta da parte que a `CJ1` recebe no sorteio.
